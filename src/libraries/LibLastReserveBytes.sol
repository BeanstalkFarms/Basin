...
library LibLastReserveBytes {
    ...
    function storeLastReserves(bytes32 slot, uint40 lastTimestamp, uint256[] memory reserves) internal {
     ...
     */
    function readLastReserves(bytes32 slot) internal view returns (uint8 n, uint40 lastTimestamp, uint[] memory reserves) {
        ...
        reserves = new uint[](n);
        assembly { mstore(add(reserves, 32), shr(152, shl(48, temp))) }
        if (n == 1) return (n, lastTimestamp, reserves);
        assembly { mstore(add(reserves, 64), shr(152, shl(152, temp))) }
