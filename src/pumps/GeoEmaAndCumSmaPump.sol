...
contract GeoEmaAndCumSmaPump is IPump, IInstantaneousPump, ICumulativePump {
    ...
    constructor(uint _maxPercentChange, uint _blockTime, uint _A) {
    ...
    function update(uint[] calldata reserves, bytes calldata) external {
        ...
        for (uint i = 0; i < reserves.length; i++) {
            b.lastReserves[i] = capReserve(b.lastReserves[i], log2ToUD60x18(reserves[i]), blocksPassed);
            b.emaReserves[i] = (b.lastReserves[i] * (uUNIT - aN) + b.emaReserves[i] * aN) / uUNIT;
            unchecked {
                b.cumulativeReserves[i] += b.lastReserves[i] * deltaTimestamp;
            }
        }
    ...
    function initPump(bytes32 slot, uint40 lastTimestamp, uint[] memory reserves) internal {
        for (uint i = 0; i < reserves.length; i++) {
    ...
    function capReserve(uint lastReserve, uint reserve, uint blocksPassed) internal view returns (uint cappedReserve) {
        if (reserve < lastReserve) {
            ...
            maxReserve = maxReserve < type(uint).max - lastReserve ? lastReserve + maxReserve : type(uint).max;
    ...
    function readInstantaneousReserves(address well) public view returns (uint[] memory reserves) {
        ...
        uint aN = unwrap(powu(A, deltaTimestamp));
        for (uint i = 0; i < reserves.length; i++) {
            reserves[i] = exp2FromUD60x18((lastReserves[i] * (uUNIT - aN) + reserves[i] * aN) / uUNIT);
        }
    }
    ...
    function readLastCumulativeReserves(address well) public view returns (uint[] memory reserves) {
        bytes32 slot = fillLast12Bytes(well);
        uint8 n = slot.readN();
    ...
    function readCumulativeReserves(address well) public view returns (uint[] memory cumulativeReserves) {
        bytes32 slot = fillLast12Bytes(well);
        (uint8 n, uint40 lastTimestamp, uint[] memory lastReserves) = slot.readLastReserves();
        uint offset = getSlotsOffset(n) * 2;
        assembly {
