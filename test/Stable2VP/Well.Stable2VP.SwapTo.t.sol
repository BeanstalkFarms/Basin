...
contract WellStable2VPSwapToTest is SwapHelper {
    function setUp() public {
        // init 1000e18, 1100e18
        setupStable2WellWithVirtualPrice(1.1e6, 0);
    }

    function test_getSwapIn() public view {
        uint256 amountOut = 110 * 1e18;
    ...
    function test_swapTo_revertIf_sameToken() public prank(user) {
        ...
        well.swapTo(tokens[0], tokens[0], 110 * 1e18, 0, user, type(uint256).max);
    }
    ...
    function testFuzz_swapTo_equalSwap(uint256 token0AmtOut) public prank(user) {
        ...
        console.log("token0AmtOut: %s", token0AmtOut);
        console.log("------------------------------");
