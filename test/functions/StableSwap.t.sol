...
contract CurveStableSwapTest is WellFunctionHelper {
    ...
    function testFuzz_calcLpTokenSupply(uint[2] memory _reserves) public {
        ...
        uint[] memory underlying = _function.calcLPTokenUnderlying(
            lpTokenSupply,
            reserves,
            lpTokenSupply,
            _data
    ...
    function testFuzz_stableSwap(uint x, uint y, uint a) public {
        ...
        uint lpTokenSupply = _function.calcLpTokenSupply(reserves, _data);
        uint reserve0 = _function.calcReserve(
            reserves,
            0,
            lpTokenSupply,
            _data
        );
        uint reserve1 = _function.calcReserve(
            reserves,
            1,
            lpTokenSupply,
            _data
    ...
    function test_calcRateStable6Decimals() public {
        _data = abi.encode(
            CurveStableSwap2.WellFunctionData(
                10,
                address(tokens[0]),
                address(deployMockTokenWithDecimals(1, 6))
            )
        );
        uint256[] memory reserves = new uint256[](2);
        reserves[0] = 100e18;
